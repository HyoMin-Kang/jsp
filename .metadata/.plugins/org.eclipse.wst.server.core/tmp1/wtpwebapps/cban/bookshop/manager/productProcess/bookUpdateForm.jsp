	  ...
	  <div class="form-group row">
			...
			<div class="col-sm-4">
	    		<select class="form-control" id="book_kind" name="book_kind">
	    			<optgroup label="국내도서">
						<option value="100"<%if (book.getBook_kind().equals("100")) {%>selected<%} %>>건강 취미</option>
						<option value="110"<%if (book.getBook_kind().equals("110")) {%>selected<%} %>>경제 경영</option>
						<option value="120"<%if (book.getBook_kind().equals("120")) {%>selected<%} %>>문학</option>
						<option value="130"<%if (book.getBook_kind().equals("130")) {%>selected<%} %>>사회 정치</option>
						<option value="140"<%if (book.getBook_kind().equals("140")) {%>selected<%} %>>자기계발</option>
						<option value="150"<%if (book.getBook_kind().equals("150")) {%>selected<%} %>>자연과학</option>
						<option value="160"<%if (book.getBook_kind().equals("160")) {%>selected<%} %>>참고서</option>
					</optgroup>
	    			<optgroup label="외국도서">
						<option value="200"<%if (book.getBook_kind().equals("200")) {%>selected<%} %>>문학 소설</option>
						<option value="210"<%if (book.getBook_kind().equals("210")) {%>selected<%} %>>경제 경영</option>
						<option value="220"<%if (book.getBook_kind().equals("220")) {%>selected<%} %>>인문 사회</option>
						<option value="230"<%if (book.getBook_kind().equals("230")) {%>selected<%} %>>예술 대중문화</option>
						<option value="240"<%if (book.getBook_kind().equals("240")) {%>selected<%} %>>취미 라이프스타일</option>
						<option value="250"<%if (book.getBook_kind().equals("250")) {%>selected<%} %>>컴퓨터</option>
						<option value="260"<%if (book.getBook_kind().equals("260")) {%>selected<%} %>>자연과학</option>
					</optgroup>
	    		</select>
	  		</div>
		</div>
